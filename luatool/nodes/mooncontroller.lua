--
-- Register (lua) moon controller for luatool
--
...
local nodenameprefix = "mooncontroller:mooncontroller"

-- mooncontroller, 16 different nodes
local nodes = {}
for i=0,15 do
...
function definition:info(node, pos, player, itemstack)
	...
	return ns.info(pos, player, itemstack, mem, "Moon controller")
end
...
function definition:copy(node, pos, player)
	...
	return {
		description = string.format("Moon controller at %s", minetest.pos_to_string(pos)),
		code = code,
	}
end

function definition:paste(node, pos, player, data)
	-- restore settings and update lua/moon controller, no api available
	local meta = minetest.get_meta(pos)
